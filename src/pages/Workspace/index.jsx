...
import { obterEquipe } from "../../services/equipeService";
import { ACTIVE_WORKSPACE_STORAGE_KEY } from "../WorkspaceAccess/data";
...
export default function Workspace() {
  ...
  useEffect(() => {
    ...
    obterEquipe(workspaceId)
      .then((equipe) => {
        if (isCurrentRequest) {
          setCurrentWorkspace(equipe);
        }
      })
      .catch((error) => {
        console.error("Não foi possível carregar o workspace:", error);
        if (isCurrentRequest) {
  ...
  const canShareInviteCode = currentWorkspace?.role === "owner";
  ...
  return (
    <>
        ...
        <div className="workspace-container">
            ...
            <div className="workspace-current-area">
              ...
              <div className="workspace-current-info">
                ...
                <div>
                  ...
                  <strong>
                    {currentWorkspace?.name || "Workspace BCI"}
                  </strong>
                </div>
              </div>
              ...
              {canShareInviteCode && currentWorkspace?.inviteCode && (
                <div className="workspace-invite">
                  ...
                  <div className="workspace-invite-content">
                    ...
                    <strong>{currentWorkspace.inviteCode}</strong>
                  </div>
                  ...
                  <div className="workspace-invite-actions">
                    <button
                      ...
                      className={`workspace-invite-action ${
                        copiedInviteCode ? "is-copied" : ""
                      }`}
                      onClick={handleCopyInviteCode}
                      title="Copiar código"
                    >
                      {copiedInviteCode ? <FiCheck /> : <FiCopy />}
                      <span>{copiedInviteCode ? "COPIADO" : "COPIAR"}</span>
                    </button>
          ...
          <header className="workspace-header">
            <div className="workspace-header-content">
              <span className="workspace-eyebrow">CENTRAL DA EQUIPE</span>
            ...
            <div className="workspace-header-stats">
              <div>
                <FiMessageSquare />
                <strong>{workspace.summary.posts}</strong>
                <span>Publicações</span>
              </div>

              <div>
                <FiActivity />
                <strong>{workspace.summary.insights}</strong>
                <span>Insights</span>
              </div>
          ...
          {workspace.activeTab === "feed" && (
            <FeedSection
              ...
              availableLinkedContents={workspace.availableLinkedContents}
      ...
      {workspace.activeTab === "my-activity" && workspace.selectedPost && (
        <ThreadDrawer
          post={workspace.selectedPost}
          type={workspace.postTypes[workspace.selectedPost.type]}
