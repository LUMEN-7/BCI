  ...
  return (
    <section className="alerts-list">
      {alerts.length > 0 ? (
        alerts.map((alert) => {
          const config =
            alertTypeConfig[alert.type] || alertTypeConfig.information;
          const Icon = config.icon;
          return (
            <article
              key={alert.id}
              className={`alert-card ${!alert.read ? "is-unread" : ""}`}
              onClick={() => onAlertClick(alert)}
            >
              <div className={`alert-icon ${config.className}`}>
                <Icon />
              </div>
        ...
        <div className="alerts-empty">
          ...
          <h2>{activeFilter === "unread" ? "Tudo lido" : "Nenhum alerta"}</h2>
          <p>
            {activeFilter === "unread"
