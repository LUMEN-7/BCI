        ...
        <div className="results-grid">
          {results.map((car) => (
            <ResultCard
              key={car.id}
              car={car}
              selected={firstCar?.id === car.id || secondCar?.id === car.id}
              selectedSlot={
                firstCar?.id === car.id
                  ? "Modelo 1"
                  : secondCar?.id === car.id
...
function ResultCard({ car, selected, selectedSlot, onSelect }) {
  return (
    ...
    >
      ...
      {selected && <span className="selected-label">{selectedSlot}</span>}
    </button>
  );
}
...
function EmptyResults({ onClear }) {
  return (
    <div className="empty-box">
      ...
      <p>Tente pesquisar por outra marca, modelo, motor ou tipo de veículo.</p>
